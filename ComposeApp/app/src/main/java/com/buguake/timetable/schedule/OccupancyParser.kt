package com.buguake.timetable.schedule
 ...
 *  3. 标签 ← 无 OCR，节次号自上而下 1..K、星期列自周一起顺序编号；
 *     识别结果经人工校正界面确认，行/列命名可在校正界面调整。
...
object OccupancyParser {
    ...
    private suspend fun parseByAnchors(context: Context, src: Bitmap): Grid {
        ...
        run {
            val axis = detectAxis(px, pixels, W, H, bgGray, tableTopY, axisEdge)
                ?: throw IllegalStateException("未能定位节次轴")
            val header = detectHeader(px, W, H, bgGray, axis)
    ...
    private suspend fun detectAxis(
        ...
        data class Anchor(val y: Double, val x0: Double)
        val cand: List<Anchor> = narrow.map { Anchor(it.cy, it.x0.toDouble()) }
        ...
        // 节次号标签：无 OCR，按网格行自上而下 1..K 顺序编号（可在校正界面人工调整）
        val sectionLabels = (1..grid.size).toList()
        ...
        val debug = "texts=%d clusters=%d narrow=%d narrowCy=%s".format(
    ...
    private suspend fun detectHeader(
        ...
        // 无 OCR：星期列自周一起顺序编号（可在校正界面人工调整）
        val dayOfColumn = (1..cols.size).toList()
