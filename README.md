...
> v1.6 亮点：修复部分教务系统（超星等）登录页在桌面模式下只显示背景、登录框打不开的问题：该站登录页依赖 body 撑高布局，此前注入的固定宽度 viewport 会把它算成 0 高度而被整体裁掉；修复内嵌浏览器请求头透传：不再额外剥离 Accept-Encoding 等头，避免服务端按非浏览器客户端降级返回；内嵌浏览器补齐桌面模式文字自适应；切回手机模式复用系统原始 UA；通用适配器新增顶栏网址输入：没有默认入口时直接进入输入态，标题栏显示教务页标题，随时可改教务网址；启动速度优化：偏好首读、玻璃背景图解码、首页考试数据读取移出主线程，缓解冷启动卡顿。
